...
public final class TradeSource {
  ...
  public static StreamSource<Trade> tradeStream(int numTickers, int tradesPerSec, int maxLag) {
    ...
    return SourceBuilder
        ...
        .fillBufferFn(TradeGenerator::generateTrades).build();
  }
  ...
  private static final class TradeGenerator {
    ...
    private TradeGenerator(long numTickers, int tradesPerSec, int maxLagMillis) {
      ...
      this.pricesAndTrends = tickers.stream().collect(toMap(t -> t,
          t -> new LongLongAccumulator(50 * MONEY_SCALE_FACTOR, MONEY_SCALE_FACTOR / 10)));
    ...
    private void generateTrades(TimestampedSourceBuffer<Trade> buf) {
      ...
      while (scheduledTimeNanos <= nowNanos) {
        ...
        long tradeTimeNanos =
            scheduledTimeNanos - (maxLagNanos > 0 ? rnd.nextLong(maxLagNanos) : 0L);
        long tradeTimeMillis =
            startTimeMillis + NANOSECONDS.toMillis(tradeTimeNanos - startTimeNanos);
    ...
    private static List<String> loadTickers(long numTickers) {
      try (BufferedReader reader = new BufferedReader(new InputStreamReader(
          TradeSource.class.getResourceAsStream("/data/nasdaqlisted.txt"), UTF_8))) {
        return reader.lines().skip(1).limit(numTickers).map(l -> l.split("\\|")[0])
            .collect(toList());
      } catch (IOException e) {
